...
public interface TicketRepository extends JpaRepository<Ticket, Integer> {
     ...
     *
     * Los CAST son necesarios en PostgreSQL y deben cubrir *todas* las
     * apariciones del parámetro: si llega null sin tipo declarado, el driver lo
     * envía como bytea y la consulta muere con "function lower(bytea) does not
     * exist". MySQL sí lo tolera, por eso solo se ve en producción.
     */
    @Query("SELECT t FROM Ticket t WHERE " +
           "(CAST(:metodo AS string) IS NULL OR LOWER(t.metodoPago) = LOWER(CAST(:metodo AS string))) AND " +
           "(CAST(:q AS string) IS NULL OR LOWER(t.serieNumero) LIKE LOWER(CONCAT('%', CAST(:q AS string), '%')))")
